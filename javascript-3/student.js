...
const setGroceryListTitle = x => `${yourName} Grocery List`;
...
const checkGroceryCount = () => {
   let groceriesCount = groceries.length;
   if ( groceriesCount > 5) {
      return 'That looks like a big trip';
   } else if (groceriesCount === 1) {
      return '1 item';
   } else if (groceriesCount < 5 && groceriesCount > 1) {
      return `${groceriesCount} items`;
   }
}
